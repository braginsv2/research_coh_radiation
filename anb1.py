...
def main():
    ...
    norm_cry = 0.5795 * st.Zcry * st.Zcry
    norm_amo = 0.5795 * st.Zamo * st.Zamo
    for i in range(1, X_RANGE + 1):
        energy = _f(float(i) / 1000.0 * st.E0MeV)
        fhdl.write(f"{energy:g}\t{_f(intsum[i] / norm_cry):g}\t{_f(intdif[i]):g}\t"
                f"{_f(intinc[i] / norm_cry):g}\t{_f(intamo[i] / norm_amo):g}\t{_f(intiel[i] / norm_cry):g}\t{_f(intael[i]):g}\n")
    fhdl.close()
